...
//....ooo0000ooooo
  ...
  G4double cylinder3_height=(part2_width)*mm; // small part of collimation hole
  ...
  G4VSolid* part2Solid = new G4SubtractionSolid ("part2Solid", part2Solid_, cylinder3Solid, rotation, G4ThreeVector());
...
//                      0);                     //copy number
  ...
  G4VSolid* holderSolid__ = new G4UnionSolid ("holderSolid", part2Solid, part3Solid, 0, G4ThreeVector((part2_width/2.+part3_width/2.), 0,  0.) );

  G4VSolid* holderSolid   = new G4UnionSolid ("holderSolid", holderSolid__ , part4Solid, 0, G4ThreeVector(-(part2_width/2.+part4_width/2.),0.,0.) );
  ...
  G4VPhysicalVolume* holderPhysical =
    ...
    for(jj = 0;jj<5;jj++){
      ...
      //Ta.setX(xCopyShift + ii*2*(fiberCoat_Rmax + fiber_dd));
        Ta.setX(ii*2*(fiberCoat_Rmax + fiber_dd));
        }
        else{
      ...
      //Ta.setX(xCopyShift + ii*2*(fiberCoat_Rmax + fiber_dd));
        ...
        //Ta.setY(fiber_Y0 + jj*2*(fiberCoat_Rmax + fiber_dd)*TMath::Cos(30.0*deg));
        //Ta.setY(fiber_Y0 + jj*2*(fiberCoat_Rmax + fiber_dd));
        Ta.setY(jj*2*(fiberCoat_Rmax + fiber_dd));
        Ta.setZ(0);
        Tr = G4Transform3D(Ra, Ta);
        fiberCorr.physical = new G4PVPlacement( Tr, fiberCorr.logical, "Sector", world.logical, false, 0);
        fiberClad.physical = new G4PVPlacement( Tr, fiberClad.logical, "SectorClad", world.logical, false, 0);
